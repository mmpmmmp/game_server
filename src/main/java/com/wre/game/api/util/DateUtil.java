...
import java.util.*;
/*
 * 由于为了以后使用方便,所有方法的返回类型都设为了 Date 请在使用时根据自己的需要进行日期格式化处理,如:
 *
...
public class DateUtil {
    ...
    // 获取上月的结束时间
    public static Date getEndDayMonth(int monthAdd) {
        Calendar calendar = Calendar.getInstance();
        calendar.set(getNowYear(), getNowMonth() - 1 + monthAdd, 1);
        int day = calendar.getActualMaximum(5);
        calendar.set(getNowYear(), getNowMonth() - 1 - monthAdd, day);
        return getDayEndTime(calendar.getTime());
    }
